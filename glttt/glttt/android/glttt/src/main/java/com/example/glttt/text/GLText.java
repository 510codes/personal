...
import com.example.glttt.shader.TextureRegion;
...
public class GLText {
    ...
    public GLText(AssetManager assets) {
        ...
        batch = new SpriteBatch();  // Create Sprite Batch (with Defined Size)
    ...
    public boolean load(ISpriteShader spriteShader, String file, int size, int padX, int padY) {
        ...
        textureId = spriteShader.loadTexture(bitmap);
    ...
    // R: [none]
    public void draw(ISpriteShader spriteShader, String text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ, float[] vpMatrix, float[] colour)  {
        ...
        for (int i = 0; i < len; i++)  {              // FOR Each Character in String
            ...
            //Log.d("GLText", "draw(): constructing sprite: letterX: " + letterX + ", letterY: " + letterY + ", chrWidth: " + chrWidth + ", chrHeight + " + chrHeight + ", charRgn[" + c + "]: " + charRgn[c]);
            sprites[i] = new Sprite(letterX, letterY, chrWidth, chrHeight, charRgn[c], modelMatrix);
    ...
    public void draw(ISpriteShader spriteShader, String text, float x, float y, float z, float angleDegZ, float[] vpMatrix, float[] colour) {
        draw(spriteShader, text, x, y, z, 0, 0, angleDegZ, vpMatrix, colour);
    }
    public void draw(ISpriteShader spriteShader, String text, float x, float y, float angleDeg, float[] vpMatrix, float[] colour) {
        draw(spriteShader, text, x, y, 0, angleDeg, vpMatrix, colour);
    }

    public void draw(ISpriteShader spriteShader, String text, float x, float y, float[] vpMatrix, float[] colour) {
        draw(spriteShader, text, x, y, 0, 0, vpMatrix, colour);
    }
    ...
    public float drawC(ISpriteShader spriteShader, String text, float x, float y, float angleDeg, float[] vpMatrix, float[] colour) {
    ...
    public void drawTexture(int width, int height, float[] vpMatrix, ISpriteShader spriteShader)  {
        float[] colour = {1.0f, 1.0f, 1.0f, 1.0f};

        float[] idMatrix = new float[16];
        Matrix.setIdentityM(idMatrix, 0);
        Sprite sprite = new Sprite(width - (textureSize / 2), height - ( textureSize / 2 ), textureSize, textureSize, textureRgn, idMatrix);
        batch.drawSprites(spriteShader, new Sprite[]{sprite}, vpMatrix, textureId, colour);
    }
}